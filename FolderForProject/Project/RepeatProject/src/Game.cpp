...
void Game::init()
{
	...
	m_tankSprite.setTextureRect(sf::IntRect(224, 0, 114, 246));
	m_tankSprite.setOrigin(m_tankSprite.getGlobalBounds().width / 2, m_tankSprite.getGlobalBounds().height / 2);
	m_tankSprite.setPosition(m_window.getSize().x / 2, m_window.getSize().y / 2);
	m_tankSprite.setRotation(90.f); // overwrites rotation of the tank 
	m_tankSprite.rotate(90.f); // Rotate the tank sprite in adittion of its current rotation
